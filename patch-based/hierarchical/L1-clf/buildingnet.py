...
network = conv_2d(network, 64, 5, strides=1, activation='relu', regularizer='L2', name = 'conv1_3_3_4', weights_init='Xavier')
network = conv_2d(network, 64, 5, strides=1, activation='relu', regularizer='L2', name = 'conv1_3_3_5', weights_init='Xavier')
#network = batch_normalization(network)
network = conv_2d(network, 64, 5, strides=1, activation='relu', regularizer='L2', name = 'conv1_3_3_6', weights_init='Xavier')
network = max_pool_2d(network, 2, strides=2)
network = conv_2d(network, 64, 5, strides=1, activation='relu', regularizer='L2', name = 'conv1_3_3_7', weights_init='Xavier')
network = max_pool_2d(network, 2, strides=2)
...
network = regression(network, optimizer='adam',
                     ...
                     learning_rate=0.00001, metric=acc)

# The model with details on where to save
# Will save in current directory
model = tflearn.DNN(network, checkpoint_path='/home/kgadira/NN-Slum/filtered-model-2-cl-final/model-', max_checkpoints = 3, tensorboard_verbose=1)

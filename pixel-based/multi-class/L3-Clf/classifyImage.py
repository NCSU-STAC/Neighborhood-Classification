...
for clf_path in clf_names: 
    ...
    f = '/home/kgadira/results/{}-clf-L3-{}-{}.bin.npy'.format(clf_path, MININDEX, MAXINDEX)
    if os.path.isfile(f):
        print 'Processing already completed for{} for the image'.format(clf_path)
        continue
    model = pickle.load(open('../models/final-L3-{}-model.sav'.format(clf_path),'rb'))
    start_t = time.time()
    ...
    f = '/home/kgadira/results/{}-clf-L3-{}-{}.bin'.format(clf_path, MININDEX, MAXINDEX)

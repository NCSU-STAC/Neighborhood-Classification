'''
Generate optimal hyperparameters using filtered training data from L2 classification
@author: Krishna Karthik Gadiraju/kkgadiraju
Source: Scikit-learn tutorials and API from scikit-learn website, stackoverflow
'''
...
# Read files 
trainX = np.load('/scratch/slums/bl-slums/gt/filtered-L2-L3-tr-X')
trainY = np.load('/scratch/slums/bl-slums/gt/filtered-L2-L3-tr-Y')
testX = np.load('/scratch/slums/bl-slums/gt/filtered-L2-L3-te-X')
testY = np.load('/scratch/slums/bl-slums/gt/filtered-L2-L3-te-Y')
...
#Count the number of elements per class in training and testing
num_labels = int(np.max(testY))
print 'Number of labels = {}'.format(num_labels)
for i in range(4):
    n_train = np.sum(trainY == i)
    n_test = np.sum(testY == i)
    print 'Number of elements of class {}: Train = {}, Test = {}'.format(i, n_train, n_test)

# Utility function to report best scores
# Source: Scikit learn website and stackoverflow
...
#XGB CLassifier
model = xgboost.XGBClassifier(nthread=8 ,objective='multi:softmax')

"""
Given the large size of the raster, not always possible to fit entire raster into memory to classify
Better to do it in a distributed manner
This code takes as input a starting row index, ending row index and processes those particular rows and writes those results to disk as a numpy array
We use predict.c to call this method to run each of these sub-parts of the image on a separate machine and finally combine all results into a single array
We can then use python scripts from postprocessing folder to combine the numpy arrays on a larger machine (stac-lab1)
...
for clf_path in clf_names: 
    ...
    f = '/home/kgadira/results/{}-filtered-clf-L3-{}-{}.bin'.format(clf_path, MININDEX, MAXINDEX)

'''
Detect changes (new informal settlements) by comparing VHR image from 2016 (informal settlement mask) against ETM-Landsat 7 image from 2002 (urban mask)
If there was no urban construction in 2002 at a location where it is identified as informal in 2016, note this as a change in the 2016 result
    ...
    for col in xrange(0,cols1, 30):
        ...
        if etm_row < rows2 and etm_col < cols2:
            ...
            if (np.shape(vhr_data)[0] > 0):
                if(etm_data != 0): # we are only looking for those slum instances that are categorized as open land
...
write_image(cols1, rows1, 1,transform1, result_raster, '/scratch/slums/bl-slums/change-detection/xgboost-slum-detected-changes.tif')               
